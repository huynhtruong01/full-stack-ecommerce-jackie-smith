import { Box } from '@mui/material'
import { useEffect, useState } from 'react'
import { useNavigate, useParams } from 'react-router-dom'
import { toast, ToastContainer } from 'react-toastify'
import 'react-toastify/dist/ReactToastify.css'
import colorsApi from '../../../api/colorsApi'
import ColorsForm from '../components/ColorsForm'

ColorsUpdate.propTypes = {}

function ColorsUpdate() {
    const { id } = useParams()
    const navigate = useNavigate()
    const [color, setColor] = useState({})

    useEffect(() => {
        const getColor = async () => {
            try {
                const color = await colorsApi.getById(id)
                setColor(color)
            } catch (error) {
                console.log(error)
            }
        }

        getColor()
    }, [id])

    const onSubmit = async (values) => {
        try {
            const { message } = await colorsApi.update({ ...values, _id: id })
            toast.success(message, {
                autoClose: 2000,
                theme: 'colored',
            })

            setTimeout(() => navigate('/colors'), 3000)
        } catch (error) {
            toast.error(error.response.data.message, {
                autoClose: 2000,
                theme: 'colored',
            })
        }
    }

    return (
        <Box width="100%" p="10px" display="flex" justifyContent="center">
            <Box width="400px">
                <Box p="20px" backgroundColor="#fff" borderRadius="8px">
                    {Object.keys(color).length && <ColorsForm values={color} onSubmit={onSubmit} />}
                </Box>
            </Box>
            <ToastContainer />
        </Box>
    )
}

export default ColorsUpdate
